...
\def\PageRightMargin{1.3cm}  % Lề phải (Cân bằng để fix lỗi căn giữa)
...
% Header
$if(header_line)$
    \fancyhead[L]{\fontfamily{ptm}\selectfont\textcolor{gray}{\small $header_line$}} % XÓA \vspace{3pt}
$else$
    \fancyhead[L]{\fontfamily{ptm}\selectfont\textcolor{gray}{\small $title$}}       % XÓA \vspace{3pt}
$endif$
...
% Footer
$if(footer_line)$
    % Nếu có biến footer_line từ YAML, sử dụng nó
    \fancyfoot[L]{\fontfamily{ptm}\selectfont\textcolor{gray}{\small $footer_line$}}
$else$
    % Nếu không, sử dụng các biến \def mặc định
...
\setlist{
    leftmargin=*,    % Giữ nguyên căn lề trái thông minh
    itemsep=6pt,     % <--- TĂNG GIÁ TRỊ NÀY (Khoảng cách giữa các dòng 1, 2, 3...)
    topsep=6pt,      % Khoảng cách giữa danh sách và đoạn văn phía trên
    parsep=3pt       % Khoảng cách giữa các đoạn văn trong cùng 1 item (nếu item dài)
}
...
% 2. Danh sách Bảng (List of Tables)
% Kéo danh sách bảng sang trái tương tự
\setlength{\cfttabindent}{-\NumberWidth}

% 3. Danh sách Hình (List of Figures)
% Kéo danh sách hình sang trái tương tự
\setlength{\cftfigindent}{-\NumberWidth}
...
\begin{longtable}{|p{0.09\textwidth}|p{0.30\textwidth}|p{0.14\textwidth}|l|p{0.16\textwidth}|l|}
    \hline
